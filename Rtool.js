...
let Rtn = ($.isNode() ? process.env.Rtn : $.getdata('Rtn')) || '';//姓名  ，只能是是环境变量，或者按照上面的来，将54行
                       Rtn = RtnArr[index];
                        Rts = RtsArr[index];
                        Rtp = RtpArr[index];
                        这三个注释掉
...
let acadamy = ""; //学院
let className = ""; //班级
let Rtn = ($.isNode() ? process.env.Rtn : $.getdata('Rtn')) || ''; //姓名环境变量
let Rts = ($.isNode() ? process.env.Rts : $.getdata('Rts')) || ''; //学号环境变量
let Rtp = ($.isNode() ? process.env.Rtp : $.getdata('Rtp')) || ''; //手机号环境变量
let wechatNumber = "";
    ...
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                getmsgLength = getmsg.length
                console.log(`\n该报名表格含有${getmsgLength}条数据~`)
                // getmsg = JSON.stringify(getmsg)
                for (let i = 0; i < getmsgLength; i++) {
