...
感谢：https://github.com/yml2213
参考：https://github.com/ygxiuming/Lecture-registration、

参数说明：
let acadamy = "";//学院 ，要在  ""  里面填写相关信息
let Rtn = ($.isNode() ? process.env.Rtn : $.getdata('Rtn')) || '';//姓名  ，只能是是环境变量，或者按照上面的来，将74行
                       Rtn = RtnArr[index];
                        Rts = RtsArr[index];
                        Rtp = RtpArr[index];
                        这三个注释掉
...
let RThd = $.getdata('RThd')

let acadamy = "";//学院
let className = "";//班级
let Rtn = ($.isNode() ? process.env.Rtn : $.getdata('Rtn')) || '';//姓名
let Rts = ($.isNode() ? process.env.Rts : $.getdata('Rts')) || '';//学号
let Rtp = ($.isNode() ? process.env.Rtp : $.getdata('Rtp')) || '';//手机号
let wechatNumber = "";
        ...
        console.log(
            `\n\n=============================================== 脚本执行 - 北京时间(UTC+8)：${new Date(
                ...
                8 * 60 * 60 * 1000
                    ...
                    let RtnArr = await checkEnv(Rtn, "Rtn");
                    let RtsArr = await checkEnv(Rts, "Rts");
                    let RtpArr = await checkEnv(Rtp, "Rtp")
                  //  let Rt_tokenArr = await checkEnv(Rt_token, "Rt_token")
                    console.log(`\n========== 共找到 ${RtnArr.length} 个账号 ==========`)
                    console.log(`这是你的账号数组:\n ${RtnArr}`);
                    for (let index = 0; index < RtnArr.length; index++) {
                        let num = index + 1;
                        console.log(`\n========== 共找到 ${num} 个账号 ==========`)
                        Rtn = RtnArr[index];
                        Rts = RtsArr[index];
                        Rtp = RtpArr[index];
                        // Rt_token = Rt_tokenArr[index];
...
function RToolck() {
    if ($request.url.indexOf("detail?") > -1) {
        ...
        $.msg($.name, "", `Rtool${status}获取${RToolurl}成功`)
...
function getkey(timeout = 0) {
    return new Promise((resolve) => {
        ...
        Rt_token = RTurl.match(/access_token=(.*?)&/)[1];
...
function sendinfo(timeout = 0) {
    return new Promise((resolve) => {
        ...
        for (let i = 0; i < getmsgArr; i++) {
            ...
            }else if(ecoU(msg_field_name[i]) == "%E5%90%8D%E5%AD%97"){//名字
                msg_field_value[i] = Rname
            }else if(ecoU(msg_field_name[i]) == "%E5%90%8D%E7%A7%B0"){//名称
                msg_field_value[i] = Rname
            }else if(ecoU(msg_field_name[i]) == "%E6%89%8B%E6%9C%BA%E5%8F%B7"){//手机号
                msg_field_value[i] = Rphone
            }else if(ecoU(msg_field_name[i]) == "%E7%94%B5%E8%AF%9D"){//电话
                msg_field_value[i] = Rphone
            }else if(ecoU(msg_field_name[i]) == "%E6%89%8B%E6%9C%BA"){//手机
                msg_field_value[i] = Rphone
            }else if(ecoU(msg_field_name[i]) == "%E6%89%8B%E6%9C%BA%E5%8F%B7%E7%A0%81"){//手机号码
                msg_field_value[i] = Rphone
            }else if(ecoU(msg_field_name[i]) == "%E8%81%94%E7%B3%BB%E6%96%B9%E5%BC%8F"){//联系方式
                msg_field_value[i] = Rphone
            }else if(ecoU(msg_field_name[i]) == "%E7%8F%AD%E7%BA%A7"){//班级
                msg_field_value[i] = className
            }else if(ecoU(msg_field_name[i]) == "%E5%B9%B4%E7%BA%A7"){//班级
                msg_field_value[i] = className
            }else if(ecoU(msg_field_name[i]) == "%E5%AD%A6%E5%8F%B7"){//学号
                msg_field_value[i] = RID
            }else if(ecoU(msg_field_name[i]) == "%E5%B7%A5%E5%8F%B7"){//工号
                msg_field_value[i] = RID
            }else if(ecoU(msg_field_name[i]) == "%E5%AD%A6%E9%99%A2"){//学院
                msg_field_value[i] = acadamy
            }else if(ecoU(msg_field_name[i]) == "%E5%BE%AE%E4%BF%A1%E5%8F%B7"){//微信号
